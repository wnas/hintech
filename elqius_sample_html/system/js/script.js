...
$.fn.cycle.createPagerAnchor = function(i, el, $p, els, opts) {
	...
	// Array Studio : takes titles from H5 and replaces the default numbering with these titles
	var j = 0;
	var slideTitle = [];
	$('.slide h5').each(function(index){
		slideTitle[j] = $(this).text();
		j++;
	});
	...
	if ($.isFunction(opts.pagerAnchorBuilder)) {
		...
		a = '<a href="#">'+slideTitle[i]+'<span class="search-pointer"></span></a>';
...
$(document).ready(function(){
	...
	$.getScript("/system/js/livequery.js", function(){
		...
		if ($('#tabs').length>0) {
    		...
    		$('ul.bottom-more li a[href != "#"]').click(function(e){
    			e.preventDefault();
	...
	if ($(".slide-holder").length>0) {
        ...
        onPagerEvent: function(zeroBasedSlideIndex, slideElement){
          //Pause when the visitor actively selects a slide through the bottom links
          $(".slide-holder").cycle('pause');
          //Restart the timeout every time a visitor clicks; after timeout expires auto-rotation to the next slide happens immediately
          clearTimeout($(".slide-holder").data('timeout'));
          $(".slide-holder").data('timeout', setTimeout(function(){$(".slide-holder").cycle('resume');},20000));
        }
		});
